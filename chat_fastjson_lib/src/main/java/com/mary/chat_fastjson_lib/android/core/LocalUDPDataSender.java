...
import com.mary.chat_fastjson_lib.android.utils.UDPUtils;
import com.mary.chat_fastjson_lib.server.protocal.CharsetHelper;
import com.mary.chat_fastjson_lib.server.protocal.ErrorCode;
...
/**
 * File Name:	LocalUDPDataSender
 * Author:      Mary
 * Write Dates: 2016/2/18
 * Description:	数据发送处理实用类
 * Change Log:
...
//				ClientCoreSDK.getInstance().setLoginHasInit(false);
	...
	public static abstract class SendCommonDataAsync extends AsyncTask<Object, Integer, Integer> {
		...
		public SendCommonDataAsync(Context context, Protocal p) {
			if (p == null) {
				Log.w(LocalUDPDataSender.TAG, "无效的参数p==null!");
				return;
			}
			this.context = context;
			this.p = p;
	...
	public static abstract class SendLoginDataAsync extends AsyncTask<Object, Integer, Integer> {
		...
		protected void onPostExecute(Integer code) {
			if (code.intValue() == 0) {
				...
				Log.d(LocalUDPDataSender.TAG, "数据发送失败, 错误码是：" + code + "！");
			}
