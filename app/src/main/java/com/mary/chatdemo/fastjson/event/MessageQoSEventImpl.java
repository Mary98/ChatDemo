...
public class MessageQoSEventImpl implements MessageQoSEvent {
	...
	public void messagesLost(ArrayList<Protocal> lostMessages) {
		...
		if(this.activity != null) {
			((TwoActivity)this.activity).showIMInfo_brightred("[消息未成功送达]共"+lostMessages.size()+"条!(网络状况不佳或对方id不存在)");
		}
	}
	...
	public void messagesBeReceived(String fingerPrint) {
		if(fingerPrint != null) {
			...
			if(this.activity != null) {
				((TwoActivity)this.activity).showIMInfo_blue("[收到对方消息应答]fp=" + fingerPrint);
			}
		}
	}
