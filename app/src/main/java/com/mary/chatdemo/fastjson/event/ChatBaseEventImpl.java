...
//			Log.i(TAG, "【DEBUG_UI】登录成功，当前分配的user_id=！"+dwUserId);
            ...
            if(this.activity != null) {
                ((TwoActivity)this.activity).refreshMyid();
                ((TwoActivity)this.activity).showIMInfo_green("登录成功,id=" + userId);
            }
        } else {
            ...
            if(this.activity != null) {
                ((TwoActivity)this.activity).refreshMyid();
                ((TwoActivity)this.activity).showIMInfo_red("登录失败,code=" + errorCode);
            }
        }
    ...
    public void onLinkCloseMessage(int dwErrorCode) {
        ...
        if(this.activity != null) {
            ((TwoActivity)this.activity).refreshMyid();
            ((TwoActivity)this.activity).showIMInfo_red("服务器连接已断开,error=" + dwErrorCode);
        }
    }
